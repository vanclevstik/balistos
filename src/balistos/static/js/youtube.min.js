...
$(document).click(function(){$('#response').hide();});$(document).on("click","#response",function(event){event.stopPropagation();});function showResponse(response){$("#response").html("");$.each(response.items,function(index,value){$("#response").append('<li data-title="'+value.snippet.title+'" data-image="'+value.snippet.thumbnails.default.url+'"" title="Add '+
...
value.snippet.title+'</div></li>');ko.applyBindings(playlist,$("#video-"+value.id.videoId)[0]);});$("#response").show();}
...
else{delaySearch($(this).val());}}});$(document).on("click",".overlay.mute",function(){player.mute();$(".overlay").removeClass("mute").addClass("unmute");});$(document).on("click",".overlay.unmute",function(){player.unMute();$(".overlay").removeClass("unmute").addClass("mute");});$("#video-id").bind("DOMSubtreeModified",function(){if($(this).text()!==""){if(player){player.loadVideoById(playlist.firstVideoId(),0,"large");$("title").text("Balistos - "+playlist.firstVideoTitle());}
else{initPlayer();}}});initPlayer();}
function initPlayer(){if(!player){if(playlist.firstVideoId()&&playlist.firstVideoStart()){$("title").text("Balistos - "+playlist.firstVideoTitle());player=new YT.Player("player",{height:"390",width:"640",videoId:playlist.firstVideoId(),playerVars:{controls:0,showinfo:0,start:parseInt(playlist.firstVideoStart()),disablekb:1,iv_load_policy:3,wmode:"transparent",rel:0},events:{"onReady":onPlayerReady}});}
