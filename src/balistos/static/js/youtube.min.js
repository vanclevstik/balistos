...
function search(query){var request=gapi.client.youtube.search.list({part:'snippet',q:query,type:'video',maxResults:8,format:5,videoEmbeddable:true,key:'AIzaSyCnR3Vv-Erxjaa-IJapIXCnvgTOuXLXItA'});request.execute(showResponse);}
...
else{delaySearch($(this).val());}}});$(document).on("click",".overlay.play",function(){player.playVideo();$(".overlay").removeClass("play").addClass("pause");});$(document).on("click",".overlay.pause",function(){player.pauseVideo();$(".overlay").removeClass("pause").addClass("play");});$(document).on("click",".overlay.mute",function(){player.mute();$(".overlay").removeClass("mute").addClass("unmute");});$(document).on("click",".overlay.unmute",function(){player.unMute();$(".overlay").removeClass("unmute").addClass("mute");});$("#video-id").bind("DOMSubtreeModified",function(){if($(this).text()!==""){if(player){var start=parseInt($("#video-start").text());player.loadVideoById($(this).text(),0,"large");}
else{initPlayer();}}});initPlayer();}
function initPlayer(){var video=$("#video-id").text();var start=parseInt($("#video-start").text());if(video!==""){player=new YT.Player("player",{height:"390",width:"640",videoId:video,playerVars:{controls:0,showinfo:0,start:start,disablekb:1,iv_load_policy:3,wmode:"transparent",rel:0},events:{"onReady":onPlayerReady}});}
