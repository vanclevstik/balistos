(function($){"use strict";
 $("#hide-player").click(function(){$(this).hide();$(".player").hide();$("#show-player").show();if(player){player.destroy();player=null;}});$("#show-player").click(function(){$(this).hide();$(".player").fadeIn();$("#hide-player").show();initPlayer();});$(document).click(function(){$('#response-playlist').hide();});$(document).on("click","#response-playlist",function(event){event.stopPropagation();});$(document).on("click",".protected_playlist",function(event){event.preventDefault();$(this).parents("li").find("form").fadeToggle();});$(document).on("submit",".protected_playlist_form",function(event){var uri=$(this).find("input").attr("data-uri");var item=$(this).parents("li");event.preventDefault();$.ajax({type:"GET",url:"/authenticate_playlist",dataType:"json",data:{"password":$(this).find("input").val(),"uri":uri}}).done(function(data){if(data.success){window.location.href="/playlist/"+uri;}
else if(error){item.find(".playlist_password_error").text(data.error);}});});$("#search_playlist").keyup(function(){$.ajax({type:"GET",url:"/search_playlists",dataType:"json",data:{"query":$(this).val()}}).done(function(data){if(data.length<1){$("#response-playlist").html('No results for this query...');$("#response-playlist").show();}
...
else{$("#registration-repeat-error").text("Passwords don't match.");$("#register-repeat").addClass("red");$("#register-form").find("input[type='password']").val("");}}});$("#chat-form").submit(function(){$.ajax({type:"POST",url:"/chat_message",dataType:"json",data:$(this).serialize(),}).done(function(data){playlist.sync();});$(this).find("textarea").val("");return false;});$("#chat-message").keydown(function(event){var keypressed=event.keyCode||event.which;if(keypressed==13){event.preventDefault();$("#chat-form").submit();}});$("#volume-slider").simpleSlider();$("#volume-slider").bind("slider:changed",function(event,data){if(player){player.setVolume(parseInt(data.value*100));}});$(".speaker").click(function(){$("#volume-slider").simpleSlider("setValue",0);if(player){player.setVolume(0);}});function validateEmail(email){return/^.+@.+\..+$/.test(email);}
$(".user").click(function(e){e.stopPropagation();$("#logout").slideDown();});$(".open-login").click(function(e){e.stopPropagation();$(".menu-button").removeClass("active");$(".open-login").addClass("active");$(".dropdown").hide();$("#login").slideDown();});$(".open-register").click(function(e){e.stopPropagation();$(".menu-button").removeClass("active");$(".open-register").addClass("active");$(".dropdown").hide();$("#register").slideDown();});$(".open-create").click(function(e){e.stopPropagation();$(".menu-button").removeClass("active");$(".open-register").addClass("active");$(".dropdown").hide();$("#create").slideDown();});$(".dropdown").click(function(e){e.stopPropagation();});$(document).click(function(){$(".menu-button").removeClass("active");$(".dropdown").hide();});$(".share-icon.facebook").click(function(){window.open('http://www.facebook.com/sharer.php?u='+$(location).attr('href'),encodeURI($(document).attr('title')),'width=600,height=300');});$(".share-icon.twitter").click(function(){window.open('http://twitter.com/home?status='+$(location).attr('href')+'+'+encodeURI($(document).attr('title')),$(location).attr('href'),'width=600,height=300');});$(".share-icon.google").click(function(){window.open('https://plus.google.com/share?url='+$(location).attr('href'),encodeURI($(document).attr('title')),'width=600,height=300');});}(jQuery));